...
//float biases[12] = {13,21,  26,17,  29,31,  45,21,  20,52,  45,46}; //sarship
//float biases[12] = {24,23,  35,45,  62,62,  157,33,  38,183,  118,135}; //504
...
void forward_yolo_layer(const float* input, const layer l)
{
	...
	//cout << *((float*)input + 16320-1) << endl;
	//cout << *(l.output +l.outputs*l.batch * sizeof(float)/4-2) << endl;
	//cout << *(l.output+16320-1) << endl;
...
detection* get_detections(vector<Blob> blobs, int img_w, int img_h, int *nboxes)
{
	...
	for (int i = 0; i < blobs.size(); ++i) {
		...
		//Blob &b = blobs[i];
		//cout << blobs[i].width << endl;
		//cout << &blobs[i].data[0][0][0][0] << endl;
		//cout << &blobs[i].data[0][255][7][7] << endl;
		//cout << blobs[i].data[0] << endl;
		//cout << blobs[i].data[65270] << endl;
		forward_yolo_layer(&blobs[i].data[0], l_params);////////////////////////////////////////////////////////////////////zhiz
	}
	//blobs[i]->gpu_data()是个指针类型。
